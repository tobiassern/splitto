...
import {
	...
	update_default_currency_schema,
	userTable,
	sessionTable
} from '$lib/schema';
...
export const load: PageServerLoad = async (event) => {
	...
	return {
		...
		update_default_currency_form: await superValidate(user, zod(update_default_currency_schema))
	};
};
...
export const actions: Actions = {
	...
	'update-default-currency': async (event) => {
		const { user } = isAuthenticated(event);

		const update_default_currency_form = await superValidate(
			event,
			zod(update_default_currency_schema)
		);

		if (!update_default_currency_form.valid) {
			return fail(400, { update_default_currency_form });
		}

		await event.locals.db
			.update(userTable)
			.set(update_default_currency_form.data)
			.where(eq(userTable.id, user.id));

		return { update_default_currency_form };
	},
